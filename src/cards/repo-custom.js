...
const createTechStackNodes = (techStack, colors, cardWidth) => {
    ...
    const nodes = techStack.map((tech) => {
...
const renderRepoCard = (repo, options = {}) => {
    ...
    const requiredWidthForTitle = Math.ceil(iconWidth + finalTitleTextWidth + leftMargin + rightMargin);
    ...
    while (wordIndex < words.length && lines.length < DESCRIPTION_MAX_LINES) {
        ...
        const effectiveMaxWidth = isLastLine ? maxLineWidth - ellipsisWidth : maxLineWidth;
        ...
        if (newWidth > effectiveMaxWidth && currentLine.length > 0) {
            ...
            } else if (isPunctuation && currentWidth + wordWidth <= effectiveMaxWidth + 10) {
    ...
    // 生成技术栈并获取高度
    const techStackResult = hasTechStack ? createTechStackNodes(techStackArray, colors, cardWidth) : { nodes: "", totalHeight: 0 };
    ...
    const calculatedHeight = titleHeight + descriptionHeight + techStackGap + techStackResult.totalHeight + starForkGap + starForkHeight + bottomPadding;

    const height = fixed_height || calculatedHeight;

    // 当有技术栈时，不显示主要语言
    const svgLanguage = (primaryLanguage && !hasTechStack)
  ...
  `);
    ...
    // 使用相对内容区域的位置（内容区域从Y=55开始）
    const techStackY = 58;  // 技术栈在描述下方，为3行描述留出足够空间
    const starForkY = hasTechStack ? 58 + techStackResult.totalHeight + 18 : (height - 35 - 55);  // Star/Fork位置，在技术栈下方，间距18px
